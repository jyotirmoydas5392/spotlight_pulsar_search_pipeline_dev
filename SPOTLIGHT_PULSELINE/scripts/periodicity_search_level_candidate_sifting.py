...
def remove_duplicate_candidates(file_path):
    ...
    try:
        ...
        for line in data_lines:
            ...
            period, pdot, dm, snr, harmonic = columns[:5]  # Extract relevant fields
            ...
            if period in unique_candidates:
                if snr > unique_candidates[period][-2]:  # Last column is SNR
                    unique_candidates[period] = (period, pdot, dm, snr, harmonic)
            else:
                unique_candidates[period] = (period, pdot, dm, snr, harmonic)

        # Convert dictionary to sorted list based on highest Period
        sorted_candidates = sorted(unique_candidates.values(), key=lambda x: -float(x[0]))  # Sort by Period descending
        
        # Write back to file
        with open(file_path, 'w') as file:
            file.write(header)  # Write the header first
            for candidate in sorted_candidates:
                file.write(f"{candidate[0]} {candidate[1]} {candidate[2]} {candidate[3]:.2f} {int(candidate[4])}\n")  # Write formatted data
...
def periodicity_search_level_sift_candidates(input_dir, output_dir, file_name, start_DM, end_DM, low_period, high_period, 
            dm_step, DM_filtering_cut_10, DM_filtering_cut_1000, SNR_cut, period_tol_init_sort, min_harmonic_no):
    """
    ...
    # Check if at least one valid file exists before proceeding
    valid_files = glob.glob(os.path.join(input_dir, f"{file_name}_DM*.dat"))

    # Define output file
    output_file = os.path.join(output_dir, f"{file_name}_all_sifted_candidates.txt")

    # If no valid files were found, write the message and exit
    if not valid_files:
        with open(output_file, "w") as file:
            file.write("No valid data found to process.\n")
        print(f"No valid data found. Exiting. Response written to {output_file}")
        return
    ...
    # Prepartion to generate the canididate array for sifting purpose...
    cand_len = []

    # Checking any mising DM file and fixing those...
    first_dm = DM_array[0]  # First DM in the array
    first_file_path = os.path.join(input_dir, f"{file_name}_DM{float(first_dm):.2f}.dat")

    # If the first file is missing, find the next available one
    if not os.path.exists(first_file_path):
        for next_dm in DM_array[1:]:  # Check the next DMs
            next_file_path = os.path.join(input_dir, f"{file_name}_DM{float(next_dm):.2f}.dat")
            if os.path.exists(next_file_path):
                shutil.copy(next_file_path, first_file_path)
                print(f"First DM file missing. Copied {next_file_path} to {first_file_path}")
                break
        else:
            print("No valid file found to copy for the first DM.")

    for dm in DM_array:
        file_dm_path0 = os.path.join(input_dir, f"{file_name}_DM{(float(dm) - float(dm_step)):.2f}.dat")
        file_dm_path1 = os.path.join(input_dir, f"{file_name}_DM{float(dm):.2f}.dat")
    ...
    for i in range(0, len(DM_array)):
        ...
        A0 = A0[A0[:, 1] > 0]
        A0 = A0[A0[:, 1] < 10**8]
        A0 = A0[A0[:, 2] >= int(min_harmonic_no)]  # Harmonic number should be greater equal to min_harmonic_no
    
        cand_array_len = len(A0[:,0])
        
        for j in range(0, int(cand_array_len)):
            if low_period <= np.multiply(np.divide(1.0, A0[j][0]), 1000) <= high_period:
            
                Period_dot_array[i][j] = 0.00000
                Period_array[i][j] = np.divide(1.0, A0[j][0]) # In seconds
                SNR_array[i][j] = A0[j][1]
                Harmonic_no_array[i][j] = int(A0[j][2])
    ...
    output_file = os.path.join(output_dir, f"{file_name}_all_sifted_candidates.txt")
    with open(output_file, "w") as file:
        file.write("Period(sec)   Pdot(s/s)  DM(pc/cc)   SNR    Harmonic_no\n")
                ...
                if SNR_array[maxima_index] >= SNR_cut:
                    with open(output_file, "a") as file:
                        file.write(f"{Period_array[maxima_index]:.10f}     "
                                   ...
                                   f"{int(Harmonic_no_array[maxima_index])}\n")
